########################################################################################################################
#                                                                                                                      #
#                        2D Poisson network convergence when interpolating and using hybriid method                    #
#                                                                                                                      #
#                                          Lionel Cheng, CERFACS, 23.09.2021                                           #
#                                                                                                                      #
########################################################################################################################
...
def run(cfg: dict, interp_type: str, interp_kind: str):
    ...
    j_its = cfg['eval']['refine_its']

    # fig_dir = Path(cfg['network']['casename']) / f'cvg_interp_hybrid_{interp_kind}_{interp_type}'
    fig_dir = Path(cfg['network']['casename']) / 'diff_res' / f'hybrid_jacobi_{j_its}_it'
    ...
    for i_nnx, nnx in enumerate(nnxs):
        ...
        # Impose res_scale because of interpolation
