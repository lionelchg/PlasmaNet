...
if __name__ == '__main__':
    ...
    # Define testing resolution
    nnxs = np.array([51, 101, 201, 401])
    j_its = config['eval']['refine_its']
    ...
    if 'networks' in config:
        ...
        for nnx in nnxs:      
            ...
            config['network']['eval']['nnx'] = nnx
            config['network']['eval']['nny'] = nny
            config['network']['arch']['args']['input_res'] = nnx
            config['network']['eval'] = config['eval']
            poisson_nn = PoissonNetwork(config['network'])
            poisson_nn.case_config(config['network']['eval'])  
            run_cases(Path(config['network']['casename']) /'various_profiles' /f'gauss_its_{j_its}'/f'{nnx}', poisson_nn, plot=True)
