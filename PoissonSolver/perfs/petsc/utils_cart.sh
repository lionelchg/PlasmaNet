...
run_solver_cart() {
    ...
    do
        mpirun -np "$1" ./src/poisson.out $nn $nn "$5"/$nn.log -ksp_type "$2" -ksp_rtol "$4" -pc_type "$3"
    done
}
...
run_cg_gamg() {
    for nn in 101 201 401 801 2001 4001
    do
        mpirun -np "$1" ./src/poisson.out $nn $nn "$3"/$nn.log -ksp_type cg -ksp_rtol "$2" -pc_type gamg
    done
}
