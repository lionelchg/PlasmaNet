...
# Define attention module issued from https://towardsdatascience.com/self-attention-in-computer-vision-2782727021f6
# Particularly we use a squeeze and excitation block structure
class _Custom_attention(nn.Module):
    """
    Custom attention layer class that manually enables to pad the input
    For now it manually performs replicate on the axis and zero elsewhere
    Although this should be easily generalized
    """
    def __init__(self, input_channels):
        super(_Custom_attention, self).__init__()
        self.linear_1 =  nn.Linear(input_channels, input_channels)
        self.relu = torch.nn.ReLU()
        self.linear_2 =  nn.Linear(input_channels, input_channels)
        self.sigmoid = torch.nn.Sigmoid()
        self.lamb_val = 1.0

    def forward(self, x):
        # The attention structure will take an initial FM block and
        # apply a "mask" to rescale the values with NN parameters
        mask_x = torch.mean(x, dim=(-1,-2))

        mask_x *= self.lamb_val

        mask_x = self.linear_1(mask_x)
        mask_x = self.relu(mask_x)
        mask_x = self.linear_2(mask_x)
        mask_x = self.sigmoid(mask_x)

        return x*mask_x.unsqueeze(-1).unsqueeze(-1)
...
class _ConvBlock(nn.Module):
    ...
    """
    def __init__(self, fmaps, block_type, kernel_size, attention=None, padding_mode='zeros',
        ...
        for i in range(len(fmaps) - 1):
            if padding_mode == 'custom':
                ...
                layers.append(nn.Conv2d(fmaps[i], fmaps[i + 1],
                    ...
                    padding_mode='zeros'))
                if attention[i]:
                    layers.append(_Custom_attention(fmaps[i + 1]))
            else:
                layers.append(nn.Conv2d(fmaps[i], fmaps[i + 1],
    ...
    def forward(self, x, lamb_val):

        for layer in  self.encode:
            if layer._get_name() == '_Custom_attention':
                layer.lamb_val = lamb_val

        return self.encode(x)


class UNet_attention(ScalesNet):
    """
    Attention. All the attention layers are specified in the config file.
    Three different options are possible when going up the U: upsample, deconvolution
    or interpolation. Only interpolation allows the network to work on different resolutions
    """
    def __init__(self, scales, kernel_sizes, input_res, attention=None,
                    padding_mode='zeros', upsample_mode='nearest'):
        super(UNet_attention, self).__init__(scales, kernel_sizes)
        ...
        # Attention blocks follow the scales structure
        att_in_fmaps = attention['scale_0'][0]

        att_down_blocks = list()
        for local_depth in range(1, self.max_scale):
            att_down_blocks.append(attention[f'scale_{local_depth:d}'][0])

        att_bottom_fmaps = attention[f'scale_{self.max_scale:d}']

        att_up_blocks = list()
        for local_depth in range(self.max_scale - 1, 0, -1):
            att_up_blocks.append(attention[f'scale_{local_depth:d}'][1])

        att_out_fmaps = attention['scale_0'][1]
        ...
        self.ConvsDown.append(_ConvBlock(in_fmaps, 'in', self.kernel_sizes[0], attention=att_in_fmaps, padding_mode=padding_mode))

        # Intermediate down layers (with MaxPool at the beginning)
        for idown, down_fmaps in enumerate(down_blocks):
            self.ConvsDown.append(_ConvBlock(down_fmaps, 'down', self.kernel_sizes[idown + 1], attention=att_down_blocks[idown],
                padding_mode=padding_mode))

        # Bottom layer (MaxPool at the beginning and Upsample/Deconv at the end)
        self.ConvBottom = _ConvBlock(bottom_fmaps, 'bottom', self.kernel_sizes[-1], attention=att_bottom_fmaps,
        ...
        for iup, up_fmaps in enumerate(up_blocks):
            self.ConvsUp.append(_ConvBlock(up_fmaps, 'up', self.kernel_sizes[-2 - iup], attention=att_up_blocks[iup],
                padding_mode=padding_mode, upsample_mode=upsample_mode, out_size=list_res.pop()))

        # Out layer
        self.ConvsUp.append(_ConvBlock(out_fmaps, 'out', self.kernel_sizes[0], attention=att_out_fmaps,
                padding_mode=padding_mode))

        # Store initial lambda value
        self.lamb_val = 1.0

    def forward(self, x):
        # List of the temporary x that are used for linking with the up branch
        inputs_down = list()
        self.lamb_val = torch.mean(x[:,1])
        lamb_val = self.lamb_val

        # Apply the down loop
        for ConvDown in self.ConvsDown:
            x = ConvDown(x, self.lamb_val)
            inputs_down.append(x)

        # Bottom part of the U
        x = self.ConvBottom(x, self.lamb_val)

        # Apply the up loop
        for ConvUp in self.ConvsUp:
            input_tmp = inputs_down.pop()
            x = ConvUp(torch.cat((x, input_tmp), dim=1), self.lamb_val)
