...
def gradient_scalar(field, dx, dy):
    """
    Calculates the field gradient (second order accurate, degraded to first order on boundaries).
    The output shape is the same as the input shape.

    Parameters
    ----------
    field : torch.Tensor
        Input 2D field: tensor of size (batch, H, W)
    ...
    -------
    Tensor
        Output gradient (scalar field)
    """

    # Create gradient tensor with shape (batch_size, 2 h, w)
    gradient = torch.zeros(field.size(0), 2, field.size(1), field.size(2)).type(field.type())

    # Check sizes
    assert field.dim() == 3 and gradient.dim() == 3, 'Dimension mismatch'
    batch_size = field.size(0)
    h, w = field.size(1), field.size(2)
    ...
    gradient[:, 1, 0, 1:-1] = (4 * field[:, 1, 1:-1] - 3 * field[:, 0, 1:-1] - field[:, 2, 1:-1]) / (2 * dy)
    gradient[:, 1, -1, 1:-1] =  (3 * field[:, -1, 1:-1] - 4 * field[:, -2, 1:-1] + field[:, -3, 1:-1]) / \
                                 (2 * dy)
    gradient[:, 0, 1:-1, 0] = (4 * field[:, 1:-1, 1] - 3 * field[:, 1:-1, 0] - field[:, 1:-1, 2]) / \
    gradient[:, 0, 1:-1, -1] = (3 * field[:, 1:-1, -1] - 4 * field[:, 1:-1, -2] + field[:, 1:-1, -3]) / \

    # corners (respectively upper left, upper right, lower left and lower right)
    gradient[:, 0, 0, 0] = (4 * field[:, 0, 1] - 3 * field[:, 0, 0] - field[:, 0, 2]) / (2 * dx) + \
    gradient[:, 1, 0, 0] = (4 * field[:, 1, 0] - 3 * field[:, 0, 0] - field[:, 2, 0]) / (2 * dy)
    gradient[:, 0, -1, 0] = (4 * field[:, -1, 1] - 3 * field[:, -1, 0] - field[:, -1, 2]) / (2 * dx) + \
    gradient[:, 1, -1, 0] =   (3 * field[:, -1, 0] - 4 * field[:, -2, 0] + field[:, -3, 0]) / (2 * dy)
    gradient[:, 0, 0, -1] = (3 * field[:, 0, -1] - 4 * field[:, 0, -2] + field[:, 0, -3]) / (2 * dx) +\
    gradient[:, 1, 0, -1] =  (4 * field[:, 1, -1] - 3 * field[:, 0, -1] - field[:, 2, -1]) / (2 * dy)
    gradient[:, 0, -1, -1] = (3 * field[:, -1, -1] - 4 * field[:, -1, -2] + field[:, -1, -3]) / (2 * dx) + \
    gradient[:, 1, -1, -1] =  (3 * field[:, -1, -1] - 4 * field[:, -2, -1] + field[:, -3, -1]) / (2 * dy)
