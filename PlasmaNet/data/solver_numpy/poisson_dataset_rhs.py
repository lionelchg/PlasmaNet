            ...
            for sigma_x in sigma_range:
                for sigma_y in sigma_range:
                    ...
                    if nit % 50 == 0 and plot:
                        plot_fig(X, Y, potential[nit, :, :], physical_rhs_list[nit, :, :], name='gauss/potential_2D_', nit=nit)
                    nit += 1
                    if nit % 200 == 0:
                        print('nit = %d' % nit)
        ...
        for y0 in x_range:
            for powx in pow_range:
                for powy in pow_range:
                    ...
                    physical_rhs = ni0 * parabol(X.reshape(-1), Y.reshape(-1), L) * co.e / co.epsilon_0 * cosine_hill(X.reshape(-1), Y.reshape(-1), ni0, x0, y0, powx, powy, L) * co.e / co.epsilon_0
                    rhs = - physical_rhs * dx**2
