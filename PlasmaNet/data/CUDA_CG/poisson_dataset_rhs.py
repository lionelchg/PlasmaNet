...
if __name__ == '__main__':
        ...
        for y0 in x_range:
            for sigma_x in sigma_range:
                for sigma_y in sigma_range:
                    ...
                    pot = torch.FloatTensor(potential[nit, :, :])
                    ...
                    print("Pot sum Before: ", pot.sum())
                    tol  = solveLinearSystemCG(pot,rhs_in, A_val, I_A, J_A)
                    potential[nit, :, :] = pot
                    print("Pot sum After: ", pot.sum())
                    print("tol ", tol)
    ...
    time_stop = time.time()
    np.save('../../../datasets/Ekhi_test/potential.npy', potential)
    np.save('../../../datasets/Ekhi_test/physical_rhs.npy', physical_rhs_list)
